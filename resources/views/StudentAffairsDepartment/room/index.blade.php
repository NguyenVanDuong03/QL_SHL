            ...
            <div class="card-body">
                <div class="table-responsive">
                    <table class="table table-bordered table-hover" id="classroomsTable" width="100%" cellspacing="0">
                        ...
                        @else
                            @foreach($rooms['data'] ?? [] as $room)
                                <tr data-id="{{ $room['id'] ?? $loop->iteration }}" data-name="{{ $room['name'] }}" data-status="{{ $room['status'] }}">
                                    <td>{{ $loop->iteration }}</td>
                                    <td>{{ $room['name'] }}</td>
                                    <td>
                                        <span class="badge {{ $room['status'] == 0 ? 'bg-success' : 'bg-warning' }}">{{ $room['status'] == 0 ? 'Trống' : 'Đang sử dụng' }}</span>
                ...
                <div class="d-flex justify-content-between align-items-center mt-3">
                    <div>
                        <span>Hiển thị {{ $rooms['from'] ?? 1 }} đến {{ $rooms['to'] ?? count($rooms['data'] ?? []) }} của {{ $rooms['total'] ?? count($rooms['data'] ?? []) }} bản ghi</span>
                    </div>
                    @if(!empty($rooms['links']))
                        <nav aria-label="Page navigation">
                            <ul class="pagination">
                                @foreach($rooms['links'] as $link)
    ...
    <div class="modal fade" id="addRoomModal" tabindex="-1" aria-labelledby="addRoomModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Hủy</button>
                    <button type="button" class="btn btn-primary" id="saveNewRoom">Lưu</button>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="modal fade" id="editRoomModal" tabindex="-1" aria-labelledby="editRoomModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                ...
                <div class="modal-footer">
                    <button type="button" class="btn btn-secondary" data-bs-dismiss="modal">Hủy</button>
                    <button type="button" class="btn btn-primary" id="saveEditRoom">Lưu thay đổi</button>
                </div>
            </div>
        </div>
    </div>
